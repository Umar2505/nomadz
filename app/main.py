...
def sanitizer(input: SanitizerInput) -> dict:
    ...
    try:
        ...
        if analyzer is None or anonymizer is None:
            ...
            sanitized_text = anonymizer.anonymize(text=text, analyzer_results=results).text

        return {"corrected_text": sanitized_text, "applied_rules": input.applied_rules}
    except Exception as e:
        logger.error(f"Error in sanitizer: {e}")
        return {"corrected_text": input.corrected_text, "applied_rules": input.applied_rules}
